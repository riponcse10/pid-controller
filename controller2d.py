...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            deltat = t - self.vars.t_previous
            error_v = v_desired - v
            
            integral_v = self.vars.integral_error_previous + error_v*deltat
            
            derivative = (error_v - self.vars.error_previous) / deltat
            
            acceleration = kp*error_v + ki*integral_v + kd*derivative
            ...
            yaw_heading_difference = yaw_path - yaw
            if yaw_heading_difference > self._pi:
            	yaw_heading_difference -= 2*np.pi
            if yaw_heading_difference < -self._pi:
            	yaw_heading_difference += 2*np.pi
            ...
            yaw_crosstrack_difference = np.arctan(K_e* cross_track_error/(v))
            
            steer_expect = yaw_heading_difference + yaw_crosstrack_difference
            print("Yaw Heading difference: " + str(yaw_heading_difference))
            print("Yaw Crosstrack difference: " + str(yaw_crosstrack_difference))
            print("Steer expect: " + str(steer_expect))
            if steer_expect > np.pi:
            	steer_expect -= self._2pi
            if steer_expect < -np.pi:
            	steer_expect += self._2pi
            ...
            print("Throttle: " + str(throttle_output))
        ...
        self.vars.integral_error_previous = integral_v
